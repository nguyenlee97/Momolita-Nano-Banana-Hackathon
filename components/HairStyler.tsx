...
const Uploader = ({ onImageUpload }: { onImageUpload: (file: File) => void }) => {
    ...
    return (
        ...
        >
            ...
            <span className="text-slate-400 font-semibold">{t('hairStyler.dropImage')}</span>
            <span className="text-slate-500 text-sm mt-1">{t('hairStyler.clickToUpload')}</span>
            <input id="hairstyler-upload" type="file" className="hidden" accept="image/png, image/jpeg, image/webp" onChange={handleFileChange} />
        </label>
    );
};
...
export default function HairStyler({ onBack }: { onBack: () => void }) {
    const { t } = useLanguage();
    ...
    const [gridFormat, setGridFormat] = useState<GridFormat>('3x3');
    ...
    const handleGenerate = async (instructions?: string) => {
        ...
        const prompt = `Generate avatars of this person with different hairstyles in a ${gridFormat} grid format. The identity of the person must be perfectly preserved in every avatar.`;

        try {
            const resultUrl = await generateStyledImage(prompt, [uploadedImage], instructions);
    ...
    const handleDownload = () => {
        ...
        link.download = `momolita-ai-hairstyles.jpg`;
    ...
    const renderConfigView = () => (
        <div className="w-full grid md:grid-cols-2 gap-8 items-start">
            <div className="flex flex-col items-center gap-4">
                <h3 className="font-bold text-2xl text-neutral-200 mb-1">{t('hairStyler.uploadTitle')}</h3>
                {uploadedImage ? (
            ...
            <div className="bg-black/20 backdrop-blur-md border border-neutral-800 rounded-2xl p-6 shadow-lg flex flex-col gap-4">
                <h3 className="font-bold text-2xl text-neutral-200 mb-2">{t('hairStyler.optionsTitle')}</h3>
                <div>
                    <h4 className="font-bold text-neutral-300 mb-2">{t('hairStyler.gridFormat')}</h4>
                    <div className="grid grid-cols-3 gap-2">
                         {(['2x2', '3x3', '4x4'] as GridFormat[]).map(format => (
                            <OptionButton key={format} label={format} isSelected={gridFormat === format} onClick={() => setGridFormat(format)} />
                         ))}
                    </div>
                </div>
                <div>
                    <label htmlFor="refine-prompt-initial" className="block text-lg font-bold text-neutral-200 mb-2">{t('common.refineLabel')}</label>
                     <textarea
                        id="refine-prompt-initial"
                        value={refinePrompt}
                ...
                >
                    {isLoading ? t('hairStyler.generatingButton') : t('hairStyler.generateButton')}
                </button>
            </div>
        </div>
    );
    ...
    const renderResultView = () => (
         <div className="w-full max-w-xl flex flex-col items-center gap-8">
            <div className="w-full">
                 <h3 className="font-bold text-2xl text-neutral-200 mb-4 text-center">{t('hairStyler.resultTitle')}</h3>
                <div className="aspect-square w-full bg-black/20 rounded-lg border-2 border-dashed border-neutral-700 flex items-center justify-center text-neutral-500 text-center relative overflow-hidden">
                    ...
                    {error && !isLoading && (
                        <div className="p-4 text-red-400">
                            <p className="font-semibold mb-2">{t('hairStyler.generationFailed')}</p>
                    ...
                    {generatedImage && <img src={generatedImage} alt="Generated Hairstyles" className="w-full h-full object-contain" />}
                </div>
            </div>
            ...
            >
                ...
                <div className="text-center mb-10">
                    <h2 className="text-5xl md:text-6xl font-extrabold text-white mb-2 flex items-center justify-center gap-4 tracking-tight">
                        ...
                        {t('app.hairStylerTitle')}
                    </h2>
                    <p className="text-xl md:text-2xl text-neutral-400 mt-2">{t('hairStyler.subtitle')}</p>
                </div>

...
const Uploader = ({ onImageUpload }: { onImageUpload: (file: File) => void }) => {
    ...
    return (
        ...
        >
            ...
            <span className="text-slate-400 font-semibold">{t('illustrationToFigure.dropImage')}</span>
            <span className="text-slate-500 text-sm mt-1">{t('illustrationToFigure.clickToUpload')}</span>
            <input id="figure-upload" type="file" className="hidden" accept="image/png, image/jpeg, image/webp" onChange={handleFileChange} />
        </label>
    );
};
...
const DEFAULT_PROMPT = "turn this photo into a character figure. Behind it, place a box with the character's image printed on it, and a computer showing the Blender modeling process on its screen. In front of the box, add a round plastic base with the character figure standing on it. set the scene indoors if possible";

export default function IllustrationToFigure({ onBack }: { onBack: () => void }) {
    const { t } = useLanguage();
    ...
    const handleDownload = () => {
        ...
        link.download = 'momolita-ai-figure.png';
    ...
    const handleGenerate = async (image: string, instructions?: string) => {
        ...
        setView('result');
        try {
            const resultUrl = await generateStyledImage(DEFAULT_PROMPT, [image], instructions);
    ...
    const renderConfigView = () => (
        <div className="w-full grid md:grid-cols-2 gap-8 items-start">
             <div className="bg-black/20 backdrop-blur-md border border-neutral-800 rounded-2xl p-6 shadow-lg flex flex-col items-center">
                <h3 className="font-bold text-2xl text-neutral-200 mb-1">{t('illustrationToFigure.uploadTitle')}</h3>
                <p className="text-neutral-300 text-sm mb-4">{t('illustrationToFigure.uploadDesc')}</p>
                {uploadedImage ? (
                     <div className="relative group aspect-[4/5] w-full max-w-md rounded-md overflow-hidden">
            ...
            <div className="bg-black/20 backdrop-blur-md border border-neutral-800 rounded-2xl p-6 shadow-lg flex flex-col gap-4">
                <h3 className="font-bold text-2xl text-neutral-200 mb-1">{t('illustrationToFigure.promptTitle')}</h3>
                <p className="text-neutral-300 text-sm ">{t('illustrationToFigure.promptDesc')}</p>
                 <textarea
                    id="refine-prompt"
                    value={refinePrompt}
                ...
                >
                    {isLoading ? t('illustrationToFigure.generatingButton') : t('illustrationToFigure.generateButton')}
                </button>
            </div>
        </div>
    );
    ...
    const renderResultView = () => (
         <div className="w-full flex flex-col items-center gap-8">
            <div className="w-full grid md:grid-cols-2 gap-8">
                <ImageViewer title={t('illustrationToFigure.originalImage')} imageUrl={uploadedImage} />
                <ImageViewer title={t('illustrationToFigure.generatedFigure')} imageUrl={generatedImage}>
                     {isLoading && (
                    ...
                    {error && !isLoading && (
                         <div className="p-4 text-red-400">
                             <p className="font-semibold mb-2">{t('illustrationToFigure.generationFailed')}</p>
            ...
            >
                ...
                <div className="text-center mb-10">
                    <h2 className="text-5xl md:text-6xl font-extrabold text-white mb-2 flex items-center justify-center gap-4 tracking-tight">
                        ...
                        {t('app.illustrationToFigureTitle')}
                    </h2>
                    <p className="text-xl md:text-2xl text-neutral-400 mt-2">{t('illustrationToFigure.subtitle')}</p>
                </div>
